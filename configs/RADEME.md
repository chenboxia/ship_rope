...
| 功能 | 准确率要求 | 模块 |
|------|-----------|------|
| 系缆识别 | >=90% | rope_analyzer + state_machine + behavior_verifier |
| 救生衣识别 | >=90% | lifejacket_detector |

## 系缆识别原理

缆桩嵌入墙壁不可见，本系统通过缆绳自身形态推断系结状态：
1. YOLO11s检测三类目标：船体、缆绳、船员
2. 缆绳端点提取（骨架法/PCA法/检测框兜底），结合壁面标定推定系结锚点
3. 三维形态判定：张紧度（曲线伸直比）+ 走向角度（壁面法线夹角）+ 船侧端点距离
4. 船员行为验证：空间接近性 + 缆绳IoU
5. 五阶段状态机防误报：系结确认 -> 脱系触发 -> 船员辅助判断 -> 超时报警 -> 换缆完成
